...
/** Write a Selene integration status file to the KitchenOS vault. */
function writeStatusFile(
  vaultPath: string,
  db: DatabaseType,
  recipeCount: number
): void {
  const statusDir = join(vaultPath, 'Selene Integration');
  ...
  const activePlan = db.prepare(
    `SELECT week FROM meal_plans WHERE status = 'active' AND test_run IS NULL ORDER BY week DESC LIMIT 1`
  ).get() as { week: string } | undefined;

  const content = `---
last_indexed: ${now}
recipe_count: ${recipeCount}
active_plan: ${activePlan?.week || 'none'}
---
...
| Metric | Value |
|--------|-------|
| Recipes Indexed | ${recipeCount} |
| Last Indexed | ${now} |
| Active Meal Plan | ${activePlan?.week || 'None'} |

> This file is auto-generated by Selene. Open SeleneChat to plan meals.
`;

  writeFileSync(join(statusDir, 'Status.md'), content, 'utf-8');
  log.info({ recipeCount, activePlan: activePlan?.week }, 'Wrote status file');
}
...
if (require.main === module) {
  ...
  // Write status file to KitchenOS vault
  const vaultPath = config.kitchenOsVaultPath;
  if (vaultPath && existsSync(vaultPath)) {
    const recipeCount = results.recipes?.processed || 0;
    writeStatusFile(vaultPath, defaultDb as DatabaseType, recipeCount);
  }
